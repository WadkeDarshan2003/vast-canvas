...
const normalizePackageType = (type?: string): ProjectPackage | undefined => {
  ...
  if (normalized.includes('starter') || normalized.includes('package 1') || normalized.includes('20')) return ProjectPackage.PACKAGE_50;
  if (normalized.includes('growth') || normalized.includes('package 2') || normalized.includes('50')) return ProjectPackage.PACKAGE_100;
  if (normalized.includes('business') || normalized.includes('package 3') || normalized.includes('100')) return ProjectPackage.PACKAGE_200;
...
const normalizePackageRecord = (pkg: Partial<Package> & { type?: string; id: string }): Package => {
  const packageType = normalizePackageType(pkg.type) || ProjectPackage.PACKAGE_50;
...
export const getPackages = async (tenantId?: string): Promise<Package[]> => {
  try {
    ...
    if (tenantId) {
      q = query(collection(db, PACKAGES_COLLECTION), where('tenantId', '==', tenantId));
    }
    const packagesSnapshot = await getDocs(q);
    return packagesSnapshot.docs
      .map(doc => normalizePackageRecord({ ...(doc.data() as Partial<Package>), id: doc.id }))
      .sort((a, b) => Object.values(ProjectPackage).indexOf(a.type) - Object.values(ProjectPackage).indexOf(b.type));
  } catch (error) {
